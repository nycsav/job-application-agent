...
export function buildJobSearchQueries({ hours = 24 } = {}) {
  const time = `newer_than:${hours}h`;
  return [
    `from:(linkedin.com OR indeed.com OR my.theladders.com OR jobright.com OR dice.com OR ziprecruiter.com) ${time}`,
    `subject:(job alert OR jobs for you OR job recommendation OR role match OR career opportunity) ${time}`,
    `subject:(VP AI OR Director AI OR Head of AI OR AI transformation OR enterprise AI strategy) ${time}`,
    `subject:(AI Strategy OR AI Advisory OR AI Consulting OR Management Consulting AI OR Digital Strategy) ${time}`,
    `from:(recruiter OR talent OR hiring OR staffing OR headhunter OR search) subject:(AI OR opportunity OR role OR position OR candidate) ${time}`,
    `subject:(reaching out OR exciting opportunity OR perfect fit OR open role) (AI OR strategy OR director OR VP) ${time}`
  ];
}
