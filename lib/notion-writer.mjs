...
// Canonical shared dedup key — the SAME key every writer must use so parallel
// agents never duplicate a row (see lib/dedup.mjs + CLEANUP-WORK-ORDER.md).
import { dedupeKey, urlKey, findDuplicate, normalize } from './dedup.mjs';
...
export function buildNotionPage(role, resumeMatch = {}, options = {}) {
  ...
  // Observability cross-cut — "Agent Trail on every write" (who/when/why).
  // Pass options.agentTrail to override; otherwise we derive one. Skip with
  // options.withAgentTrail === false (e.g., a DB without the property yet).
  if (options.withAgentTrail !== false) {
    properties['Agent Trail'] = options.agentTrail || buildAgentTrail({ role, status });
  }
...
// ─── Canonical Keyed Writer (the "one keyed writer" of the architecture) ──
// Every insert into the Career Command Center goes through keyedStage(), which
// checks the SHARED dedup key first (lib/dedup.mjs) and only then builds the
// create payload. This is what lets the parallel manager + the notion-career
// Worker write the same DB without ever duplicating a row.

/**
 * Build the audit string written to the "Agent Trail" property on every row.
 * Format: <stage> · <ISO ts> · <status> · key=<id> · <why> [· note]
 */
export function buildAgentTrail({ role = {}, status = 'New', stage = 'manager', note = '' } = {}) {
  const when = new Date().toISOString();
  const jobUrl = role.apply_url || role.url || '';
  const key = urlKey(jobUrl) || dedupeKey(role.company, role.title);
  const why = `score ${role.score ?? '?'}/10` +
    (role.resume_cluster ? `, resume ${role.resume_cluster}` : '') +
    (role.source ? `, src ${role.source}` : '');
  return `${stage} · ${when} · ${status} · key=${key} · ${why}${note ? ' · ' + note : ''}`;
}

/**
 * Keyed staging — the single entry point for adding a role to the board.
 * Checks the canonical dedup key against already-known rows, and only builds a
 * create payload when there is no duplicate.
 *
 * @param {object} role         - { title|? , company, apply_url|url, score, ... }
 * @param {Array}  existingRows - rows already in Notion: { company, title, jobUrl, pageId, status }
 * @param {object} resumeMatch  - { cluster, file, angle } from resume-picker/matchResume
 * @param {object} options      - { status, note, withAgentTrail }
 * @returns {{duplicate:boolean, key:string, existingPageId?:string, reason?:string, page?:object, agentTrail?:string}}
 */
export function keyedStage(role, existingRows = [], resumeMatch = {}, options = {}) {
  const jobUrl = role.apply_url || role.url || '';
  const key = urlKey(jobUrl) || dedupeKey(role.company, role.title);
  const dup = findDuplicate(existingRows, { company: role.company, title: role.title, jobUrl });
  if (dup) {
    return {
      duplicate: true,
      key,
      existingPageId: dup.pageId || dup.id || null,
      reason: `KEYED-DEDUPE: matches existing "${dup.title}" @ ${dup.company}`,
    };
  }
  const status = options.status || (role.score >= 8 ? 'Materials Ready' : 'New');
  const agentTrail = options.agentTrail || buildAgentTrail({ role, status, note: options.note });
  const page = buildNotionPage(role, resumeMatch, { ...options, status, agentTrail });
  return { duplicate: false, key, page, agentTrail };
}
...
export function checkNotionDuplicate(searchResults, company, title) {
  ...
  for (const result of searchResults) {
    ...
    // Substring match OR canonical normalized-key match (catches "VP, AI Strategy
    // & Delivery" vs "VP AI Strategy and Delivery", stop-words, punctuation).
    const titleMatch = resultText.includes(titleLower) || titleLower.includes(resultTitle) ||
      (normTitle && normResult.includes(normTitle));
    const companyMatch = resultText.includes(companyLower) ||
      (normCompany && normResult.includes(normCompany));
